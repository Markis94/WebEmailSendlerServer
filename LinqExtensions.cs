...
namespace WebEmailSendler
{
    ...
    {
        public static IQueryable<T> ToPageList<T>(this IQueryable<T> query, int pageNumber, int pageSize, string? sortField = null, string? orderBy = null)
        {
            int take = pageSize;
            ...
            if (sortField != null)
            {
                ...
                {
                    return query.OrderByColumnDescending(sortField).Skip(skip).Take(take);
                }
                else
                {
                    return query.OrderByColumn(sortField).Skip(skip).Take(take);
                }
            }
            else
            {
